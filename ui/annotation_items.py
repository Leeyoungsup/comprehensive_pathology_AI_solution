...
class DrawingBrushItem(QGraphicsPathItem):
    """
    Brush로 그리는 임시 아이템
    - 마우스 이동으로 점들을 추가
    - 각 점을 반지름 크기의 원으로 합쳐 채워진 영역으로 표시
    - 완료 시 Path.toFillPolygon()로 다각형 좌표를 얻음
    """
    def __init__(self, radius: float = 10.0, color: QColor = QColor(0, 255, 0)):
        super().__init__()
        self.points: List[QPointF] = []
        self.radius = radius
        self.color = color
    ...
    def update_path(self):
        # Rebuild path by adding circle at each point
        path = QPainterPath()
        for p in self.points:
            path.addEllipse(p, self.radius, self.radius)
        self.path = path
        self.setPath(path)

    def get_coordinates(self) -> List[Tuple[float, float]]:
        """Fill Polygon을 이용해 경계 좌표를 반환"""
        if not self.points:
            return []
        poly = self.path.toFillPolygon()
        coords = [(pt.x(), pt.y()) for pt in poly]
        return coords
