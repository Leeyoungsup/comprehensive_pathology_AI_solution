"""
Slide information management module
Extraction and management of WSI file metadata and property information
"""
...
class SlideInfo:
    """Slide information extraction and management class"""
    ...
    def get_mpp(self):
        """Return MPP (Microns Per Pixel) information"""
    ...
    def get_objective_power(self):
        """Return objective power information"""
    ...
    def get_all_properties(self):
        """Return all property information"""
        return dict(self.properties)

    def get_complete_info(self, filename=None):
        """Return complete slide information"""
    ...
    def format_info_text(self, filename=None):
        ...
        lines.append(f"Magnification: {info['objective_power']}x")
        lines.append(f"Pixel Size (Level 0): {info['dimensions'][0]} x {info['dimensions'][1]}")

        if info['mpp_x'] and info['mpp_y']:
            lines.append(f"MPP: {info['mpp_x']:.4f} x {info['mpp_y']:.4f} \u00b5m/pixel")

        if 'physical_width_mm' in info:
            lines.append(f"Physical Size: {info['physical_width_mm']:.2f} x {info['physical_height_mm']:.2f} mm")

        lines.append(f"\nTotal Levels: {info['level_count']}")
